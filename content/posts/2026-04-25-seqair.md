...
and also how it differs from the other implementations (or not).
...
five parallel vectors that act like columns of a table.
...
This was the big idea, but along the way I had some more.
...
The interesting part isn't the block reader, though,
it's what happens when you think about how we do file I/O.
...
It's a pattern that only makes sense because the Rust type system
can enforce that the shared state is genuinely read-only after construction —
if any of it were `&mut`-accessible,
I'd either need to be really confident or use mutexes
to keep this design.
...
- zero allocations per record in the read path, after the first region
- zero allocations per record in the write path, after the first record

and a lot of compiler-enforced correctness[^head].
...
2. It works! I think.
   I have manged to get zero diff lines running this on quite big BAM files with Rastair.
...
4. Examples for a nice realignment API.
5. Also, some bonus features like a nice VCF/BCF writer API.
