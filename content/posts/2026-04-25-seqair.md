...
2. `indexing_slicing`, which disallows `x[2]`.
   ...
   In some parts of the code we deal with fixed-sized arrays, so there we allow this (after assertions!).
...
So, technically, we now have an implementation
that covers a lot of features,
uses basically zero allocation reading/writing records ammortized,
and has a lot of compiler-enforced correctness[^head]
as well as comparison tests.
